...
$valRabiaNum=false;
$valGusanosNum =false;
$valLeucemiaNum=false;
$valRabiaFelinaNum=false;
$valTripleNum=false;
$valAnemiaNum=false;
$valInfluenzaNum=false;
$valAdenitisNum=false;
$valEncefalomielitisNum=false;
...
if ($_SERVER["REQUEST_METHOD"] == "POST"){
	...
	$valor = explode('.',$_POST['select_chipeadores']);
	$nombre_chipeador = $valor[0];
	...
	if (isset($_POST['rabiaFelino']) && $_POST['rabiaFelino'] == 'RabiaFelino')
	{
		...
		{
			if(!is_numeric($_POST['txt_fecha_rabia_felino']))
			{
				$rabiaFelino='Rabia';
				$fechaRabiaFelino=test_input($_POST['txt_fecha_rabia_felino']);
			}else{
				$valRabiaFelinoNum=true;
			}
		}else{
			$valRabiaFelino = true;
		}
...
$valor = explode('.',$_POST['select_chipeadores']);
$pNombre_chipeador = $valor[0];

if(empty($propietarioPrincipal)){
	$propietarioPrincipal = test_input($_POST['txt_propietario']);
}
...
//Chipeadores
if ( $pNombre_chipeador) {
	$nombre_chipeador = $pNombre_chipeador;
}
else {
	$pNombre_chipeador = "";
}
...
{
	$fk_id_ejemplar = sql_insert_ejemplar($nombreEjemplar,$anioNacimiento,$especie,$raza,$pelaje,$tamanio,$alzada,$libreta,$sexo,$condicion,$desparacitado,$caracter,$capturas,$castrado,$fechaCastrado,$observaciones,$nroChip, $fechaChip, $plan, $nroRecibo, $sextuple, $fechaSextuple, $quintuple, $fechaQuintuple, $rabia, $fechaRabia, $hidatidosis, $fechaHidatidosis,
				...
				<form id="form1" name="form1" method="post" onsubmit="" action="<?php $_SERVER["PHP_SELF"];?>">
					...
					<div class="row">
						<div class="col-md-12 col-md-offset">
							...
							<?php
								if($pEspecie == "Canina" && ($valSextuple == true || $valQuintuple == true || $valHidatidosis == true || $valRabia == true || $valGusanos == true)){
									echo "
								...
								}elseif($pEspecie == "Felina" && ($valLeucemiaNum == true || $valRabiaFelinoNum == true || $valTripleNum == true)){
									echo "
										<div class='alert alert-danger-alt alert-dismissable'>
										<span class='glyphicon glyphicon-exclamation-sign'></span>
										<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>
								...
								}elseif($pEspecie == "Equina" && ($valAnemiaNum == true || $valInfluenzaNum == true || $valAdenitisNum == true || $valEncefalomielitisNum == true)){
									echo "
										<div class='alert alert-danger-alt alert-dismissable'>
										<span class='glyphicon glyphicon-exclamation-sign'></span>
										<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>
										×</button>La FECHA de aplicacion de las VACUNAS debe ser seleccionada con el calendario.</div>
									";
									$valAnemiaNum = false;
									$valInfluenzaNum = false;
									$valAdenitisNum = false;
												...
												<div class="form-group">
													<div class="input-group">
														<span class="input-group-addon">
															<input type="checkbox" name="desparacitado" <?php if(isset($_POST['desparacitado']) && $_POST['desparacitado']){ echo "checked='checked'"; }?> /> <label>¿Desparacitado?</label>
														</span>
													</div>
												</div>
